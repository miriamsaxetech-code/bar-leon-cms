...
assert.match(lunch.html, /signature-dishes/);
assert.match(lunch.html, /Sabores de Andalucía/);
assert.match(lunch.html, /site-header/);
assert.match(lunch.html, /bar-leon-hero-fajalauza\.webp/);
assert.match(lunch.html, /bar-leon-habas\.webp/);
assert.match(lunch.html, /bar-leon-callos\.webp/);
assert.match(lunch.html, /bar-leon-sesos\.webp/);
assert.match(lunch.html, /bar-leon-archivo-caricatura\.webp/);
assert.match(lunch.html, /bar-leon-archivo-fundador\.webp/);
assert.match(lunch.html, /bar-leon-fachada\.webp/);
assert.match(lunch.html, /home-cta-nav/);
assert.match(lunch.html, /loading="lazy"/);
assert.match(lunch.html, /home-hero__place/);
assert.match(lunch.html, /home-cta-btn--primary">Ver la carta<\/a>/);
assert.equal((lunch.html.match(/home-cta-btn--primary/g) || []).length, 1, 'homepage has one dominant CTA');
assert.equal((lunch.html.match(/class="signature-dish"/g) || []).length, 3, 'homepage shows exactly three house dishes');
assert.match(lunch.html, /home-quick-links/);
assert.doesNotMatch(lunch.html, /chalkboard-menu|pizarra-dia|home-hero__lion/);
assert.doesNotMatch(lunch.html, /class="call-number"/);
assert.match(lunch.html, /home-quick-link--call/);
assert.doesNotMatch(lunch.html, />\+34 958 22 51 43<\/a>/);
assert.doesNotMatch(lunch.html, /hemeroteca/i);
assert.ok(lunch.html.indexOf('home-cta-nav') < lunch.html.indexOf('signature-dishes'), 'CTA group must appear before food');
...
assert.match(english.html, /home-cta-btn--primary">View full menu<\/a>/);
...
assert.match(french.html, /home-cta-btn--primary">Consultez la carte<\/a>/);
